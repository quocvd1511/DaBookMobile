...
export default function SearchBook(){
    ...
    React.useEffect(() => 
  {
    async function fetchData(){
      const request = await axios.get('http://192.168.1.4:3000/search/' + name)
      setBook(request.data.books)
      return request.data.books
    }
    fetchData();

    },['http://192.168.1.4:3000/search/name'])

	 console.log(Book);

   function BookViewed(index){
    console.log(username + ' SÁCH ĐÃ XEM ' + Book[index].tensach)
    const request = axios.get('http://192.168.1.4:3000/sachdaxem?username=' + username + '&tensach=' + Book[index].tensach + '&hinhanh=' + Book[index].hinhanh + '&giaban=' + Book[index].giaban);
